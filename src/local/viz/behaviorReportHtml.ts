...
.cfg{margin:0 0 12px;padding:8px 12px;border:1px solid var(--bd);border-radius:7px;background:var(--s1);font:11px/1.55 var(--mono);color:var(--muted)}
.cfg b{color:var(--ink2);font-weight:600}
.cfg.cfg-warn{border-color:var(--rbd);background:var(--rbg);color:var(--red)}
.cfg.cfg-tuned{border-color:var(--obd);background:var(--obg);color:var(--ink2)}
.cfg-row{display:block;margin-top:3px}
...
.delta-chip.dc-click{cursor:pointer;text-decoration:underline;text-decoration-style:dotted;text-underline-offset:2px;transition:opacity .15s}
.delta-chip.dc-click:hover{opacity:.7}
...
.delta-hero .dh-link{cursor:pointer;text-decoration:underline;text-decoration-style:dotted;text-underline-offset:2px;transition:color .15s}
.delta-hero .dh-link:hover{color:var(--orange)}
.delta-hero .dh-stat .dh-stat-link{cursor:pointer;text-decoration:underline;text-decoration-style:dotted;text-underline-offset:2px;transition:color .15s}
.delta-hero .dh-stat .dh-stat-link:hover{color:var(--orange)}
.risk-card.highlight-pulse{animation:riskPulse .6s ease 2}
@keyframes riskPulse{0%,100%{box-shadow:none}50%{box-shadow:0 0 0 3px var(--orange)}}
...
(function(){
  ...
  var chips=[];
  function chip(txt,cls,dataAttr){chips.push('<span class="delta-chip '+(cls||'')+(dataAttr?' dc-click':'')+'"'+(dataAttr?' '+dataAttr:'')+'>'+txt+'</span>');}
  chip('Since '+ago+':','dc-none');
  if(d.totalDelta)chip((d.totalDelta>0?'+':'')+d.totalDelta+' behaviors');
  if(d.provenDelta)chip((d.provenDelta>0?'+':'')+d.provenDelta+' proven',d.provenDelta>0?'dc-up':'dc-risk','data-action="proven"');
  if(d.associatedDelta)chip((d.associatedDelta>0?'+':'')+d.associatedDelta+' test signal','','data-action="signal"');
  if(d.generatedDelta)chip((d.generatedDelta>0?'+':'')+d.generatedDelta+' generated tests',d.generatedDelta>0?'dc-up':'');
  d.newRisks.slice(0,3).forEach(function(p){chip('new in top-20: '+p.slice(0,40),'dc-risk','data-path="'+p.replace(/"/g,'&quot;')+'"');});
  ...
  el2.addEventListener('click',function(e){
    ...
    if(c){scrollToRisk(c.getAttribute('data-path'));return;}
...
(function(){
  ...
  if(d.newRisks.length>0){
    ...
    d.newRisks.forEach(function(r){parts.push('<li class="dh-new dh-link" data-path="'+r.replace(/"/g,'&quot;')+'">' +r.slice(0,60)+'</li>');});
    parts.push('</ul>');
  } else {
  ...
  if(d.provenDelta)stats.push('<span class="dh-stat-link" data-action="proven">'+((d.provenDelta>0?'+':'')+d.provenDelta+' proven')+'</span>');
  ...
  hero.addEventListener('click',function(e){
    ...
    if(li){scrollToRisk(li.getAttribute('data-path'));return;}
    var stat=e.target.closest('[data-action="proven"]');
    if(stat){var pr=D.risks.find(function(r){return r.tags&&r.tags.some(function(t){return t[0]==='Proven'||t[0]==='Dynamically Proven'});});if(pr)scrollToRisk(pr.path);}
  });
})();
...
renderRiskFilters();
renderRisks();
// ── Config disclosure: what the per-repo config did. Always rendered — a tuned
// report must never pass as clean, and a broken config must announce itself.
(function(){
  const C=D.configDisclosure, host=$("#config-disclosure");
  if(!C||!host)return;
  const warn=(C.warnings||[]).length>0;
  const tuned=C.overridesActive>0||(C.rankExcludePaths||[]).length>0||!C.floor||!C.silence;
  ...
  (C.warnings||[]).forEach(w=>{html+='<span class="cfg-row">⚠ '+esc(w)+'</span>';});
  host.className="cfg"+(warn?" cfg-warn":tuned?" cfg-tuned":"");
  host.innerHTML=html;
})();
